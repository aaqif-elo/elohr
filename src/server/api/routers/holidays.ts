...
export const holidaysRouter = createTRPCRouter({
  ...
  convertToHoliday: adminProcedure
    ...
    .mutation(async opts => {
      ...
      if (opts.input.type && Object.values(HolidayType).includes(opts.input.type as HolidayType)) {
  ...
  markHolidayAsAnnounced: adminProcedure
    ...
    .mutation(async opts => {
      ...
      if (isChained) {
        ...
        const results = await Promise.all(holidays.map(h => markHolidayAsAnnounced(h.id)));
